...
from notebooks.nnge.nnge import Example, NNGE, Feature, FeatureType
...
def test_categorical_dm2011_A():
    x, y = [[0, 3], [2,0], [2,1], [2,4], [4,2], [5,3]], ["A", "A", "A", "A", "A", "A"]
...
def test_categorical_dm2011_B():
    ...
    x, y = [[0, 3], [2, 0], [2, 1], [2, 4], [4, 2], [5, 3], [2,3]], ["A", "A", "A", "A", "A", "A", "B"]
    clf = NNGE()
    clf.fit(x, y)
...
def create_examples(example_values: List[Tuple[List[any], any]], features: List[Feature]) -> List[Example]:
    return [create_example(example_values, features, label) for example_values, label in example_values]
