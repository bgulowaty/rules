...
class Example:
    ...
    def get_feature_values(self):
        return [FeatureValue(feature, value) for feature, value in self.value_by_feature.items()]
...
class Statement:
    ...
    def distance_to(self, feature_value: FeatureValue):
        ...
        return self.lower_boundary - value if value <= self.lower_boundary else value - self.upper_boundary
...
class Hyperrectangle:
    ...
    def get_covering_statements(self, example: Example) -> Set[Statement]:
        return pipe(
            self.statements,
            filter(lambda statement: any([statement.covers(fw) for fw in example.get_feature_values()])),
            set
        )

    def get_statement_by_feature(self) -> Dict[Feature, Statement]:
        return {feature: list(statements)[0] for feature, statements in
                groupby(sorted(self.statements, key=lambda s: s.feature.idx), lambda statement: statement.feature)}
    ...
    def get_statement_for_feature(self, feature: Feature) -> Optional[Statement]:
        return pipe(
            ...
            list,
            lambda statements_list: None if len(statements_list) == 0 else statements_list[0]
        )
...
class NNGE:
    ...
    def feature_range(self, feature: Feature) -> float:
        return self.highest_value_by_feature[feature] - self.lowest_value_by_feature[feature]
    ...
    def distance_hyper_example(self, rect: Hyperrectangle, example: Example):
        ...
        for feature, value_in_example in example.value_by_feature.items():
            ...
            if statement_in_hyperrect is None:
                ...
                if lower <= value_in_example <= upper:
                    ...
                    to_sum.append((lower - value_in_example) / self.feature_range(feature))
                elif value_in_example > upper:
                    to_sum.append((value_in_example - upper) / self.feature_range(feature))
                else:
        ...
        non_missing_attributes = pipe(
            to_sum,
            filter(lambda val: val is not None),
            list,
            len)
    ...
    def infer_feature_type(value):
        return FeatureType.REAL if isinstance(value, int) or isinstance(value, float) else FeatureType.CATEGORICAL

    @staticmethod
    def convert_to_example(x, y) -> Example:
        value_by_feature = {
            Feature(idx, NNGE.infer_feature_type(val)): val
    ...
    def convert_all_to_examples(X, Y) -> List[Example]:
        return [NNGE.convert_to_example(x_single, y_single) for x_single, y_single in zip(X, Y)]
    ...
    def find_leftovers(self):
        return pipe(
            self.examples,
            filter(lambda e: not any([rect.covers(e) for rect in self.hyperrectangles])),
            list
        )
    ...
    def find_closest(self, example: Example) -> Optional[Tuple[Hyperrectangle, float]]:
        ...
        distance_by_rectangle = {rect: self.distance_hyper_example(rect, example) for rect in self.hyperrectangles}

        minimal_distance_rectangle = min(distance_by_rectangle, key=distance_by_rectangle.get)

        return (minimal_distance_rectangle,
                distance_by_rectangle[minimal_distance_rectangle]) if minimal_distance_rectangle is not None else None
    ...
    def create_exemplar(example: Example) -> Hyperrectangle:
        ...
        for feature, value in example.value_by_feature.items():
            if feature.kind == FeatureType.REAL:
                statements.add(Statement(feature=feature, lower_boundary=value, upper_boundary=value))
            elif feature.kind == FeatureType.CATEGORICAL:
    ...
    def split(self, rect: Hyperrectangle, example: Example) -> List[Hyperrectangle]:
        examples_with_same_labels_as_rect = pipe(
            self.examples,
            filter(lambda e: e.label == rect.label),
            list)

        # candidates
        statements_covering_example = pipe(
            rect.statements,
            filter(lambda statement: any([statement.covers(fv) for fv in example.get_feature_values()])),
            list
        )
        ...
        if len(categorical_candidates) > 1:
            categorical_candidates_by_examples_count = {
                statement: self.count_covered_examples(statement, examples_with_same_labels_as_rect) for statement in
                statements_covering_example
            }
            categorical_statement_to_split_on = min(categorical_candidates_by_examples_count,
        ...
        if len(real_candidates) > 1:
            real_candidates_by_sample_min_distance_to_margin = {
                statement: self.find_min_distance_to_margin(statement, example) for statement in real_candidates
            }
            minimal_distance = min(real_candidates_by_sample_min_distance_to_margin.values())
        ...
        if categorical_statement_to_split_on != None:
            return [self.prune_hyperrect(rect) for rect in
                    self.do_split(rect, categorical_statement_to_split_on, example)]

        elif real_statements_with_minimal_distance != None:
            if len(real_statements_with_minimal_distance) > 1:
                best_hyperects = pipe(real_statements_with_minimal_distance,
                                      map(lambda s: self.do_split(rect, s, example)),
                                      map(lambda rects:
                                          (tuple(rects), self.find_largest_number_of_examples_covered(rects))
                                          ),
                                      dict,
                ...
                return [self.prune_hyperrect(rect) for rect in
                        self.do_split(rect, real_statements_with_minimal_distance[0], example)]

        return None

    def find_largest_number_of_examples_covered(self, hyperrects: Iterable[Hyperrectangle]):
        return pipe(
            hyperrects,
    ...
    def prune_hyperrect(self, rect: Hyperrectangle) -> Hyperrectangle:
        ...
        for statement in rect.statements:
            covered_samples = pipe(
                training_examples_covered,
                filter(statement.covers_example),
                list
            )
            ...
            if statement.feature.kind == FeatureType.CATEGORICAL:
                new_statements.add(Statement(feature=statement.feature, categories=statement.categories.intersection(
            ...
            if statement.feature.kind == FeatureType.REAL:
                ...
                new_statements.add(Statement(feature=statement.feature, lower_boundary=lowest_boundary,
                                             upper_boundary=highest_boundary))

        return Hyperrectangle(statements=new_statements, label=rect.label)

    def do_split(self, rect: Hyperrectangle, statement: Statement, cause: Example) -> List[Hyperrectangle]:
        ...
        if statement.feature.kind == FeatureType.CATEGORICAL:
            ...
            categories_without_conflicting = statement.categories.difference({conflicting_category})

            if len(categories_without_conflicting) == 0:
                return [Hyperrectangle(rect_statements_without_splitter, rect.label)]

            new_statement_1 = Statement(statement.feature, categories=categories_without_conflicting)
            return [Hyperrectangle(rect_statements_without_splitter.union({new_statement_1}),
                                   label=rect.label)]
            ...
            examples_covered_by_statement = pipe(
                ...
                filter(lambda e: any([statement.covers(fw) for fw in e.get_feature_values()])),
                list
            )
            ...
            lower_statements = rect_statements_without_splitter.union({Statement(statement.feature,
                                                                                 lower_boundary=statement.lower_boundary,
                                                                                 upper_boundary=first_lower_value)})
            upper_statements = rect_statements_without_splitter.union({Statement(statement.feature,
    ...
    @staticmethod
    def count_covered_examples(statement: Statement, examples: Collection[Example]) -> int:
        return sum([any([statement.covers(fv) for fv in e.get_feature_values()]) for e in examples])

    @staticmethod
    def find_min_distance_to_margin(statement: Statement, example: Example) -> float:
        # this will always be one element, no?
        distances = [statement.distance_to(fv) for fv in example.get_feature_values() if statement.covers(fv)]
    ...
    def extend(self, rect: Hyperrectangle, example: Example) -> Hyperrectangle:
        ...
        expanded_statements = pipe(
            ...
            map(lambda statement: self.expand(statement, example.value_by_feature[statement.feature])),
            set
        )

        new_statements = rect.get_covering_statements(example).union(expanded_statements)
        return Hyperrectangle(statements=new_statements, label=rect.label)

    def expand(self, statement: Statement, value: any) -> Statement:

        if statement.feature.kind == FeatureType.CATEGORICAL:
            return Statement(statement.feature, categories=statement.categories.union({value}))
        elif statement.feature.kind == FeatureType.REAL:
            if statement.upper_boundary < value:
                return Statement(statement.feature, lower_boundary=statement.lower_boundary, upper_boundary=value)
            return Statement(statement.feature, lower_boundary=value, upper_boundary=statement.upper_boundary)
        else:
            raise Exception("Not supported feature type")
    ...
    def min_value_so_far(self, feature: Feature):
        return pipe(
            self.examples,
            map(lambda example: example.value_by_feature[feature]),
            min
        )
