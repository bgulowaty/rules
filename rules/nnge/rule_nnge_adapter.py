...
from ..nnge.nnge import (
    Hyperrectangle,
    Statement as HyperrectStatement,
    Feature as HyperrectFeature,
    FeatureType,
    Example as NNGEExample,
    NNGE,
)
...
def to_nnge_hyper(rule: Rule, x) -> Hyperrectangle:
    ...
    for feature_idx, statements in rule.get_statements_by_feature().items():
        if len(statements) != 2:
            raise Exception("nah")
        sorted_thresholds = pipe(statements, map(lambda s: s.threshold), sorted, list)
...
def to_rule(hyperrect: Hyperrectangle) -> Rule:
    ...
    for feature, statement in hyperrect.get_statement_by_feature().items():
        if statement.upper_boundary != np.inf and statement.lower_boundary != -np.inf:
            ...
            rule_statements.add(
                Statement(feature.idx, Relation.MT, statement.lower_boundary - EPS)
            )
...
def train_nnge_and_get_rules(seed: List[Rule], x_train, y_train):
    ...
    covered_sample_indicies = pipe(
        ...
        enumerate,
        filter(
            lambda idx_with_x: any(
                [r.describes(to_instance(idx_with_x[1])) for r in seed]
            )
        ),
    ...
    lowest_value_by_feature = {
        HyperrectFeature(idx, FeatureType.REAL): val
        for idx, val in enumerate(feature_lowest)
    }
    highest_value_by_feature = {
        HyperrectFeature(idx, FeatureType.REAL): val
        for idx, val in enumerate(feature_maxes)
    }
    seed_examples = [
    ...
    not_covered_example_indicies = set(range(len(x_train))).difference(
        covered_sample_indicies
    )
